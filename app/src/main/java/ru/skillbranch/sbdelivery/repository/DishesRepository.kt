...
class DishesRepository(
    ...
    override fun getDishes(): Single<List<DishEntity>> =
        api.refreshToken(RefreshToken(DeliveryRetrofitProvider.REFRESH_TOKEN))
            ...
            .subscribeOn(Schedulers.io()) // для запроса в сеть
            .observeOn(AndroidSchedulers.mainThread()) // в команде Якова так принято - из репозитория отдавать всё в main потоке
